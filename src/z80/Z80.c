...
enum CodesED
{
  ...
  IN_C_xC,OUT_xC_C,ADC_HL_BC,LD_BC_xWORDe,DB_4C,RETI,DB_4E,LD_R_A,	/* Modified by LDS: added DB_4E */
...
int ExecZ80(Z80 *R, int RunCycles)
{
  ...
  while(R->ICount>0) /* Modified by LDS */
  {
    ...
    if(R->Trace)
      if(!DebugZ80(R)) return(R->ICount); /* Modified by LDS */
#endif

    /* Read opcode and count cycles */
    I=RdZ80(R->PC.W++);
    /* Count cycles */
    R->ICount-=Cycles[I]; /* Modified by LDS */
...
#include "Codes.h"
  ...
  /* We are done */
  return(R->ICount); /* Modified by LDS */
}
#endif /* EXECZ80 */
